...
import { CLAIM_TYPE_LABEL, inferClaimType, inferClaimTypeFromText } from './claimType.ts';
...
test('inferClaimType은 수술 플래그가 우선이고 빈 fields는 manual을 유지한다 (체험 모달 호환)', () => {
  assert.equal(inferClaimType({ docType: '진료비 영수증', diag: '피부염' }, true), 'surgery');
  assert.equal(inferClaimType({ docType: '진료비 영수증', diag: '피부염' }, false), 'skin');
  assert.equal(inferClaimType({ docType: '', diag: ' ' }, false), 'manual');
  assert.equal(inferClaimType({ docType: '진료비 영수증', diag: '' }, false), 'illness');
});
