...
> 보이고, 1단계에서 뒤로가기는 랜딩으로 나가며, 접수 완료 뒤에는 뒤로가기는 빈 1단계로·「홈으로」는 랜딩으로 가서 「신청하기」를
> 다시 누를 수 없다.
...
### 왜 S6는 `replaceState`로 덮지 않고 "뒤로가기 = reset 후 빈 1단계"인가
...
**ⓑ로 한다 — S6에서 popstate가 오면 `reset`하고 `history.go(1 - e)`로 항목 [1]까지 걷어 빈 1단계에 선다.** 항목 [1]은 우리가
마운트 때 `replaceState`로 만든 것이라 **어떤 진입 경로에서든 반드시 있다.** 랜딩으로 나가는 길은 「홈으로」(3절)다.

처음(2026-09-12 구현)에는 `go(-e)`로 `/apply` 앞 항목(랜딩)까지 나가려 했다. AI 리뷰 P3: DM 링크를 새 탭·인앱 브라우저로 열면
`/apply`가 탭의 첫 항목이라 그 앞 항목이 없고, 범위 밖 `go`는 브라우저가 조용히 무시한다 — 결과가 `reset`만 된 빈 1단계에 항목은
`{ applyStep: 5 }`인 어긋난 상태였다. 가장 흔한 진입 경로에서 전제가 깨지므로, 우리가 쌓은 항목 안에서만 움직이는 ⓑ로 바꿨다.
`reset`을 먼저 하는 이유는 `go`가 무시되더라도 빈 1단계(새 멱등 키)여야 하기 때문이다. `e == 1`이면 `go`를 부르지 않는다 —
`go(0)`은 새로고침이다.
...
| ⑥ | e < 현재 == 6 | `reset` 후 `history.go(1 - e)` — 항목 [1]까지 걷어 빈 1단계에 선다(e == 1이면 go 없음). 랜딩은 「홈으로」로 |
...
기억하고 끝낸다. 맞지 않으면 큐를 비우고(기대 모델이 어긋난 것 — 무시된 `go` 등) 규칙을 처음부터 적용한다.
...
- [x] S6에서 back → 빈 1단계(접수번호·입력 없음), 한 번 더 back → 진입 전 페이지. S6 「홈으로」 → 랜딩(`/`).
      랜딩을 거쳐 온 탭과 `/apply`가 첫 항목인 새 탭 양쪽에서 같다 (AI 리뷰 P3 반영 뒤)
- [x] 3단계에서 새로고침 → 1단계. back 첫 번째는 옛 항목이 규칙 ④로 걷혀 1단계 그대로, 두 번째에 랜딩
- [x] 콘솔 에러 없음
- [x] 「← 이전」 연타(두 클릭 사이 0·4·12ms) → 5단계에서 3단계, 2로 넘어가지 않음 (AI 리뷰 P2 반영 뒤)
- [x] Vercel 프리뷰(번들 해시가 로컬 빌드와 동일)에서 3→2→1→랜딩 · S6 back · forward 확인(P3 반영 전 동작 기준). 모바일 스와이프 백은 실기기 미확인
...
2. S6 뒤로가기 = `reset` + `history.go(1 - e)`로 빈 1단계. `replaceState`로 덮지 않는다. 랜딩은 「홈으로」 (AI 리뷰 P3로 `go(-e)`→`go(1 - e)` 변경)
