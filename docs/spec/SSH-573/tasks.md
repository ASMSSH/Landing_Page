...
- [ ] `Project/src/lib/metaPixel.ts` — `pixelStartApplication` · `pixelSubmitApplication` + `window.fbq` 타입 선언. fbq 부재 시 무시
- [ ] **커밋** `feat: 메타 픽셀 기본 코드 삽입 + fbq 래퍼 추가`

## 2. 이벤트 호출부 — spec 3·4절
- [ ] `Project/src/main.tsx` — `isApplyPath` 분기에서 `pixelStartApplication()`
- [ ] `Project/src/apply/steps/StepConsent.tsx` — `submitClaim().then()` 성공 콜백에서 `pixelSubmitApplication()`
- [ ] **커밋** `feat: /apply 진입·제출 성공에 메타 픽셀 이벤트 발사`
