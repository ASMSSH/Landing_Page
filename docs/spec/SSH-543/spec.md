...
- **Figma** 데스크톱 시안 S1(빈 상태) · S1-a(OCR 채움) · S2, 모바일 시안 S1 · S1-a · S1-b(수기) · S2 —
  문안·레이아웃·상태 표현은 여기서 가져왔다. 데스크톱은 업로드 카드 + 폼이 항상 같이 보이고, 모바일은 원래
  「영수증 없이 직접 입력할게요」 버튼으로 폼을 여는 구조였는데 **1차 리뷰에서 데스크톱과 같은 구조로 모바일 시안을 고쳤다**
  (아래 「1차 리뷰 결정」)
...
아래 1~8 전부 승인. 1번은 리뷰 중에 "모바일은 시안대로 선택 화면 → 확인 화면에 폼"으로 바꾸자는 의견이 잠시 있었으나,
**웹에서 하던 흐름이 앱(모바일 폭)에서 달라지면 이상하다**는 이유로 원안(모든 폭에서 카드 + 아래 폼, 사진을 넣으면 아래 폼이
채워짐)으로 확정했다. 이에 맞춰 **Figma 모바일 S1 프레임을 카드 + 폼 구조로 수정**했고(「또는」 행·「직접 입력할게요」 버튼 제거,
S1-b의 폼 5칸을 카드 아래로), S1-b 프레임은 「(미사용)」으로 이름을 바꿔 남겨 뒀다.
