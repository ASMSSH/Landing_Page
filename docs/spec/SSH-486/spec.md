...
1. **S4 보호자 이름 예시 「예) 김민석」 → 「예) 홍길동」** (Figma S4 데스크톱·모바일도 같이)
2. **동의 전문을 본문 교체가 아니라 팝업으로.** 웹에서 전문을 열면 맨 아래까지 내려가야 「닫기」가 보여 불편하다 → 데스크톱은 가운데
   대화상자, ≤480은 아래 시트(S1-c `ReceiptPhotoDialog`와 같은 `.apply-dialog` 패턴). 머리(칩·제목·메타·✕)와 발(「닫기」)은 고정,
   본문만 스크롤. 닫는 길은 ✕·「닫기」·배경 클릭·Esc·브라우저 back 다섯 — 전부 `history.back()`으로 모아 pushState 엔트리가 남지
   않는다. `ConsentDocument.tsx` → `ConsentDialog.tsx`. 5절의 「본문 교체」 서술과 결정 1·8은 이 절이 대체한다. Figma S5-a 프레임은
   아직 본문 교체 모습 — 웹 화면을 확인받은 뒤 팝업으로 다시 그린다
