...
- [x] PR 본문 갱신(`/pr`) → `/pr-review` 별도 서브에이전트 → 1회 등록(P1·P2 없음, P3 1건: window dragover/drop 차단이 텍스트 드래그까지 막아 폼 input에 텍스트를 끌어다 놓는 브라우저 기본 동작이 깨짐 → `Files` 드래그만 막도록 고칠지 사람 확인)
